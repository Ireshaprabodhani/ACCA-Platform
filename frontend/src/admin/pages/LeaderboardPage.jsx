...
const Card = ({ className = "", children }) => (
  <div className={`rounded-2xl bg-white/10 backdrop-blur-md shadow-2xl border border-white/20 ${className}`}>
    {children}
  </div>
);
...
const Badge = ({ children, className = "" }) => (
  <span className={`inline-block rounded-full bg-white/20 text-white px-3 py-1 text-sm font-semibold shadow-md ${className}`}>
    {children}
  </span>
);
...
export default function LeaderboardPage() {
  ...
  const rankBadge = (idx) => {
    ...
    return idx < 3 ? (
      <span className={`inline-flex items-center justify-center w-8 h-8 rounded-full font-bold ${styles[idx]}`}>
        {idx + 1}
      </span>
    ) : (
  ...
  if (error)
    return <p className="text-center mt-10 text-red-300 font-medium">{error}</p>;
              ...
              <tbody className="divide-y divide-white/10">
                <AnimatePresence>
                  {leaderboard.map((item, idx) => (
                    ...
                    >
                      <td className="px-6 py-4 font-medium">{rankBadge(idx)}</td>
                      <td className="px-6 py-4">{item.userName}</td>
                      <td className="px-6 py-4 underline underline-offset-2">{item.email}</td>
